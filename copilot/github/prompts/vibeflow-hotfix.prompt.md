...
- The fix would require an operation the vibeflow-audit prompt's Rules
...
3. The Critical Gate is clean: match the fix's diff against the
   vibeflow-audit prompt's Rules Catalog.
...
| `critical-gate` | the fix requires an operation the vibeflow-audit prompt's Rules Catalog flags as blocking |
...
The prompt commits nothing. Close with:

- The outcome — done, or the HALT with its next step and "resume with the
  vibeflow-hotfix prompt, passing the doc path".
