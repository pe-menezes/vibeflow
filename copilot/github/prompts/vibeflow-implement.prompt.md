...
1. Locate it. A file path (contains `/` or ends in `.md`) is read directly.
   A feature name is matched in `.vibeflow/specs/` — exact (`<name>.md`),
   then partial; several matches → list them and ask the user to choose;
   none → list the available specs and suggest the vibeflow-gen-spec prompt.
2. Validate required sections: Definition of Done (at least 1 check), Scope
   ...
   what's absent, and point to the vibeflow-gen-spec prompt.
...
pattern guidance (suggest the vibeflow-analyze prompt) and skip to Phase 3.
Otherwise:
...
Tests: PASS. Use the vibeflow-audit prompt to get the formal verification."
...
consider fixing the gaps above, then use the vibeflow-audit prompt."
