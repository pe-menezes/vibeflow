...
the vibeflow-analyze prompt first to create the knowledge base." — don't create it by hand.

Then branch on the input: `--from` in the input goes to **Import from external
...
Read the feedback and place it in one of four categories. Ambiguous feedback is
...
of what was added. Then suggest: "Run the vibeflow-analyze prompt at the next opportunity
