---
name: 'vibeflow-audit'
description: 'Audits implementation against its DoD and project patterns. Runs tests, scans the diff for destructive ops (Critical Gate), and reports PASS / PARTIAL / FAIL. With --consolidate-hotfixes, reclassifies hotfix trace docs against the current code.'
...
7. When `.vibeflow/hotfixes/` holds hotfix docs, close the report with one
   line — `N hotfix docs not yet consolidated` — and point at the
   vibeflow-audit prompt with `--consolidate-hotfixes`. No consolidation
   state is tracked: every doc present counts. Directory absent or empty →
   no line.
...
- PARTIAL/FAIL: "See the incremental prompt pack in the audit report.
  Fix the gaps and use the vibeflow-audit prompt again."
