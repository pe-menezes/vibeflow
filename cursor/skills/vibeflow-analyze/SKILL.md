...
- `--satellite <url>` → satellite mode (below); phases 1–5 don't run. Without an
  existing `.vibeflow/index.md`, stop: "Run the vibeflow-analyze skill first to
...
the vibeflow-teach skill. If the auto content did change, regenerate the frontmatter
...
- **`confidence`** — `inferred` on creation (found by automated analysis), `validated` once a human confirms it via `--interactive` or the vibeflow-teach skill.
...
the vibeflow-teach skill survive incremental updates.
...
Phase 3.5 or the vibeflow-teach skill), it goes between `<!-- vibeflow:auto:end -->`
...
the vibeflow-gen-spec skill or the vibeflow-prompt-pack skill — patterns from
