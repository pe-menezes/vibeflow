...
description: "Fixes an observed defect with reproducible evidence in one call: writes a short trace doc before touching code, implements the fix, and proves it with a regression test that fails before the fix and passes after. Use when a defect was observed and the evidence is in hand — an error message, a failing log, real data misbehaving — wherever it surfaced, production included. Not for planned tasks (vibeflow-quick) or spec'd features (vibeflow-implement)."
---
...
the motivating case, not a gate. The discriminator against the other skills
...
- The fix would require an operation the vibeflow-audit skill's Rules
  Catalog flags at a blocking severity (the ones its verdict map turns into
  FAIL), whether foreseen in the plan or caught in the final diff scan →
  HALT `critical-gate`. The catalog is reused by reference; none of its
...
3. The Critical Gate is clean: match the fix's diff against the
   vibeflow-audit skill's Rules Catalog.
...
| `critical-gate` | the fix requires an operation the vibeflow-audit skill's Rules Catalog flags as blocking |
...
The skill commits nothing. Close with:

- The outcome — done, or the HALT with its next step and "resume with the
  vibeflow-hotfix skill, passing the doc path".
